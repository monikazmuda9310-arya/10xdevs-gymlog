...
project's scale _and_ requires zero migration work: the scaffolded repo already ships a Workers
...
- **Preview deploys**: two viable routes. (a) **Workers Builds** — Cloudflare's git integration
  ...
  preview is tolerable, but a preview pointed at the _production_ Supabase project is a live
...
| ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Worker deployed without runtime secrets → green deploy, auth silently dead on the public URL                                  | Devil's advocate                    | **H**      | **H**  | Run `wrangler secret put` for both vars before the first production deploy; ~~add a post-deploy smoke check that fetches `/auth/signin` and asserts the "Supabase not configured" banner is absent.~~ **That mitigation does not work and was measured vacuous on 2026-08-21**: the banner renders only from `?error=not_configured`, and a bare GET carries no `?error=`, so `messageForCode(null)` is `null` and there is no banner in EITHER case — the check passes in exactly the broken case. **Built instead** (`testing-environment-parity`, Phase 5): `npm run deploy` verifies the Worker's secret NAMES before publishing, then POSTs invalid credentials to the deployed URL and reads the `?error=` code — `sign_in_failed` healthy, `not_configured` secrets absent, `unexpected` secrets wrong, `rate_limited` inconclusive. Proven able to fail against a credential-less worker. Consider flipping `optional: true` to required in `astro.config.mjs` once CI supplies the vars, so misconfiguration fails loudly. |
