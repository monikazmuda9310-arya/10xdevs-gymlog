...
| S-09 | account-boundary                  | be certain no other account can reach their training, and delete their own account      | S-02, S-03, S-06, F-01, F-03                                                                                                    | US-04, NFR §no cross-account reach, NFR §own-data deletion                          | done   |
...
- **S-09: user's training is unreachable from any other account — reads, edits and deletes alike, including a request that names a workout, exercise entry or set identifier directly, verified against the stored rows — and the user can delete their own account together with all of its training data, after which none of it is retrievable** — Archived 2026-08-22 → `context/archive/2026-08-14-cross-account-isolation/` and `context/archive/2026-08-14-account-deletion/`. **Closed by hand, and the reason is worth keeping**: the slice was split into two changes on 2026-08-15, so neither folder's Change ID equals this item's `account-boundary` and `/10x-archive`'s exact-string match correctly declined to close it — a split slice's roadmap row is nobody's automatic responsibility, and this one read `in-progress` for a week after the work was deployed and verified. Lesson: **a single-column foreign key into an RLS-protected table is an ownership hole no policy will show you** — `exercise_entries.exercise_id` points into the shared catalogue, where a composite key cannot reach because `MATCH SIMPLE` equality never matches the 38 null-owner rows, so it was closed with a `security invoker` trigger whose visibility check IS the `exercises` select policy; written `security definer` the same SQL reads correctly, runs as `postgres`, sees every row and admits everything. Two more: **reading the catalogue is not measuring it** — the planning assumption that `on delete restrict` would block `delete from auth.users` was wrong, because that RESTRICT is itself an AFTER trigger queued behind the cascade removing the referencing rows, so it never fires; and closing this defect **retired the only test of an unrelated guarantee**, since `tonnage-breakdown`'s assertion 9 constructed the grafted row the new trigger now refuses at the source, leaving the `left join` load-bearing and unguarded.
