...
- **Deploy / infra:** partial — the hosting configuration is correct and named, the pipeline (`.github/workflows/ci.yml`) runs lint, typecheck, unit tests and build on `main`, and the hosting account is authenticated. The first run with the typecheck and test steps is still pending. The product has never been deployed; `context/deployment/deploy-plan.md` is written and awaiting the owner's approval.
