...
| S-02 | exercise-catalogue                | browse and search exercises, and add their own with a muscle group and bodyweight flag  | S-01, F-03                                                                                                                      | FR-011, FR-012, FR-013, FR-014, Access Control §catalogue visibility                | in-progress |
...
- **Backend / API:** server rendering is on by default. Three authentication endpoints under `src/pages/api/auth/` (form posts) and one catalogue endpoint under `src/pages/api/exercises/` (JSON, called from a hydrated island). Business logic lives in `src/lib/services/`, input validation in `src/lib/validation/` split so the zod schemas never reach the browser. **Two tables**: `profiles` and `exercises` — the latter the first to mix shared and owned rows, with 38 seeded exercises readable by every account and writable by none, and custom exercises private to their owner. Enforced in the database and proven against persisted state by `tests/integration/exercises-rls.test.ts`. Workouts and sets do not exist yet.
