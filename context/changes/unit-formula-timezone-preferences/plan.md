...
- [ ] 2.2 `preferences-derive.test.ts` passes, including the different-record-holder assertion
- [ ] 2.3 Mutation (a): the same fixture at ten repetitions cannot distinguish the formulas
- [ ] 2.4 Mutation (b): re-stamping `weight_unit` in `updateSet` fails the existing-rows assertion
