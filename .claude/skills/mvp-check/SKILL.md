...
name: mvp-check
description: >
  Analyse the repository in the current working directory against the five
  minimal technical requirements for a 10xDevs certification MVP — CRUD,
  business logic, tests mapped to a defined risk, authentication tied to a
  user, and documentation — and return a checklist, a score out of five, and
  actionable guidance for anything unmet. Read-only. Use when the user asks to
  "check the MVP", "sprawdź projekt pod wymagania", "mvp check", or wants to
  know whether the project clears the certification bar before submitting.
allowed-tools:
  - Read
  - Glob
  - Grep
...
<!--
  Provided by the 10xDevs course instructors; the body below is theirs, kept
  verbatim. Only this comment and the frontmatter above were added, so the file
  could be registered as a skill and survive outside the throwaway worktree it
  arrived in (C:\10xdev\gymlog-account-deletion, 2026-08-22).
-->
