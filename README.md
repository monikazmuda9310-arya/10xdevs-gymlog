...
- **Nothing is deleted by halves.** The whole removal is one transaction, so a deletion that fails
  leaves every row exactly as it was — which is what the failure message says.
